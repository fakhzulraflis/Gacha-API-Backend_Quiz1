...
Ini adalah endpoint untuk melakukan gacha agar mendapatkan listed prizes.
untuk mendapatkan response pada bagian **body (JSON):** harus diisi/diinput dengan:
```json
  {
    "username": "nama_kamuu"
  }
...
Endpoint ini berguna untuk mengecek history yang telah dilakukan user. Endpoint ini memerlukan query parameter yang perlu diinput untuk menentukan user(secara spesifik) mana yang ingin dilihat history nya.
**Query Parameter:**
- `username` (wajib) - nama user yang ingin dilihat historinya

**Contoh:** `GET /gacha/history?username=fakhzulS`
...
```json
{
  "success": true,
  "prizes": [
    { "no": 1, "prize": "Emas 10 gram", "totalQuota": 1, "winnersCount": 0, "remaining": 1 },
    { "no": 2, "prize": "Smartphone X", "totalQuota": 5, "winnersCount": 2, "remaining": 3 }
  ]
}
```
